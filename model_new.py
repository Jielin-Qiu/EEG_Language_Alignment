...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        if args.modality == 'text':
            self.text_encoder = Encoder(d_feature_text, n_layers, n_head, d_k, d_v, d_model, d_inner, dropout)
            self.linear1_cov_text = nn.Conv1d(d_feature_text, 1, kernel_size=1)

        elif args.modality == 'eeg':
            self.eeg_encoder = Encoder(d_feature_eeg, n_layers, n_head, d_k, d_v, d_model, d_inner, dropout)
