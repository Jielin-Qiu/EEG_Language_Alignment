...
class EEGTransformer(nn.Module):
    ''' A sequence to sequence model with attention mechanism. '''
    def __init__(
            self, device, d_feature_eeg, d_model, 
            d_inner, n_layers, n_head, d_k, d_v, dropout, class_num, args):
    ...
    def forward(self, eeg_src_seq = None):
        
        b_eeg, l_eeg = eeg_src_seq.size()
        src_pos_eeg = torch.LongTensor(
            [list(range(1, l_eeg + 1)) for i in range(b_eeg)]).to(self.device)
        
        enc_output_eeg, *_ = self.eeg_encoder(src_seq = eeg_src_seq, src_pos= src_pos_eeg)
...
class TextTransformer(nn.Module):
    ''' A sequence to sequence model with attention mechanism. '''
    def __init__(
            self, device, d_feature_text, d_model, 
            d_inner, n_layers, n_head, d_k, d_v, dropout, class_num, args):
    ...
    def forward(self, text_src_seq = None):
